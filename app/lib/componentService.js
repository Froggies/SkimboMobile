...
exports.createConnectionWebView = function(providerName) {
	...
	webview.addEventListener('load', function(e) {
		...
		if (!_.isUndefined(cookies) && !_.isNull(cookies)) {
			var cookiesTab = cookies.split(";");
			Ti.API.info('WebviewLoad ' + cookies);
			for ( i = 0; i <= cookiesTab.length - 1; i++) {
				var tabCookie = cookiesTab[i].trim().split('=');
				if (tabCookie[0] == 'tokenSkimbo') {
					Ti.API.info('WebviewLoad get the cookie');
					var application = Alloy.createController('application');
					application.getView().open();
					Titanium.App.Properties.setString("userToken", tabCookie[1]);
